...
description: Python 惯用法、PEP 8 标准、类型提示以及构建健壮、高效和可维护 Python 应用的最佳实践。
origin: ECC
---

# Python 开发模式

编写健壮、高效和可维护应用的 Python 惯用模式和最佳实践。
...
def load_config(path: str) -> Config:
    try:
        ...
        raise ConfigError(f"配置中 JSON 无效：{path}") from e
...
class User:
    ...
    def __post_init__(self):
        ...
        if "@" not in self.email:
            raise ValueError(f"无效的电子邮件：{self.email}")
        # 验证年龄范围
...
**记住**：Python 代码应该可读、显式，并遵循最小意外原则。如有疑问，优先选择清晰而非聪明。