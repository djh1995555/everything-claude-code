...
你正在帮助翻译签证申请文件，用于签证申请。

## 说明

当用户提供图像文件路径时，自动执行以下步骤，无需确认：

1. **图像转换**：如果文件是 HEIC 格式，使用 `sips -s format png <input> --out <output>` 将其转换为 PNG 格式

2. **图像旋转**：
   - 检查 EXIF 方向数据
   - 根据 EXIF 数据自动旋转图像
   - 如果 EXIF 方向为 6，逆时针旋转 90 度
   - 根据需要应用额外旋转（如果文档显示倒置，测试 180 度旋转）
...
非常适合需要翻译文件的澳大利亚、美国、加拿大、英国和其他国家的签证申请。